...
export class MailService {
  ...
  async sendReimbursementApproved(
    ...
    if (!isEnabled) {
      this.logger.debug(
        `[DISABLED] Would send reimbursement approved email to ${to}`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(`Sent reimbursement approved email to ${to}`);
    } catch (error) {
      this.logger.error(
        `Failed to send reimbursement approved email to ${to}:`,
        error.message,
      );
    }
  }
  ...
  async sendReimbursementRejected(
    ...
    if (!isEnabled) {
      this.logger.debug(
        `[DISABLED] Would send reimbursement rejected email to ${to}`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(`Sent reimbursement rejected email to ${to}`);
    } catch (error) {
      this.logger.error(
        `Failed to send reimbursement rejected email to ${to}:`,
        error.message,
      );
    }
  }
  ...
  /**
   * Send salary advance / loan approval
   */
  async sendAdvanceLoanApproved(
    to: string,
    data: {
      employeeName: string;
    ...
    if (!isEnabled) {
      this.logger.debug(
        `[DISABLED] Would send advance/loan approved email to ${to}`,
      );
      return;
    }
    ...
    try {
      await this.mailerService.sendMail({
        to,
        subject: `${typeLabel} Request Approved`,
        template: './advance-loan-approved',
        context: { ...data, isLoan, typeLabel, ...company },
      });
      this.logger.log(`Sent advance/loan approved email to ${to}`);
    } catch (error) {
      this.logger.error(
        `Failed to send advance/loan approved email to ${to}:`,
        error.message,
      );
    }
  }
  ...
  /**
   * Send salary advance / loan rejection
   */
  async sendAdvanceLoanRejected(
    to: string,
    data: {
      employeeName: string;
      type: string; // 'ADVANCE' | 'LOAN'
    ...
    if (!isEnabled) {
      this.logger.debug(
        `[DISABLED] Would send advance/loan rejected email to ${to}`,
      );
      return;
    }

    const typeLabel = data.type === 'LOAN' ? 'Loan' : 'Salary Advance';
    const company = await this.getCompanyContext();
    try {
      await this.mailerService.sendMail({
        to,
        subject: `${typeLabel} Request Rejected`,
        template: './advance-loan-rejected',
        context: { ...data, typeLabel, ...company },
      });
      this.logger.log(`Sent advance/loan rejected email to ${to}`);
    } catch (error) {
      this.logger.error(
        `Failed to send advance/loan rejected email to ${to}:`,
        error.message,
      );
    }
  }
