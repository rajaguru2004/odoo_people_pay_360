...
export interface ClearanceStatus {
  /** Everything is clear: no assets still held. */
  cleared: boolean;
...
export class ClearanceService {
   ...
   * R27): the projection queried `assetAssignment` by a raw id, and that model
   * is `'relation'`-scoped by the HOLDER. So an id that belonged to nobody, and
   * an id that belonged to somebody in another branch, both matched zero rows
   * and both produced
  ...
  async assertCleared(
    ...
    if (!override.reason?.trim()) {
      throw new BadRequestException(
        `Cannot complete offboarding: employee still has ${status.openAssets.length} company asset(s) — ${listed}. ` +
