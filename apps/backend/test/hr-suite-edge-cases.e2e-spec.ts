...
describe('HR suite edge cases (e2e)', () => {
  ...
  describe('travel', () => {
    it('treats a same-day trip as one per-diem day, not zero', async () => {
      const res = await ctx
        .http()
        ...
        .send({
          purpose: 'Same-day client visit',
          travelType: 'DOMESTIC',
          destination: 'GCC',
          departureDate: iso(30),
          returnDate: iso(30),
          estimatedCost: 100,
        })
        .expect(201);
    ...
    it('accepts a destination with no configured rate without failing the trip', async () => {
      const label = `Unrated ${runId}`;
      await ctx.prisma.libraryItem.create({
        data: { libraryType: 'PER_DIEM_DESTINATION', label, isActive: true },
      });
      ...
      const res = await ctx
        ...
        .query({ employeeId: empAId })
        .send({
          purpose: 'Unrated destination',
          travelType: 'DOMESTIC',
          destination: label,
          departureDate: iso(32),
          returnDate: iso(33),
          estimatedCost: 50,
        })
        .expect(201);
      ...
      // A missing rate does not fail the trip; there is simply nothing to
      // claim. (The trip waits for an approver like any other — travel stopped
      // approving itself on submit, see docs/TEST-PLAN-FINANCE.md F9.)
      expect(trip?.status).toBe('PENDING');
      await ctx
        .http()
        .post(`/travel-requests/${res.body.data.id}/approve`)
        .set(bearer(adminToken))
        .send({})
        .expect(201);
      expect(trip?.perDiemRate).toBeNull();
    ...
    it('rejects a free-text destination that is not in the master', async () => {
      // Nothing enforces the label server-side today, so this documents the
      // behaviour: an unknown destination is accepted but yields no rate, and
      // therefore no per-diem. It must never crash or half-approve.
      const res = await ctx
        .http()
        .post('/travel-requests')
        .set(bearer(adminToken))
        .query({ employeeId: empAId })
        .send({
          purpose: 'Typo destination',
          travelType: 'DOMESTIC',
          destination: 'Gcc',
          departureDate: iso(34),
          returnDate: iso(35),
          estimatedCost: 50,
        })
        .expect(201);
      ...
      expect(trip?.perDiemRate).toBeNull();
      // Accepted, then decided by a human — a typo costs the per-diem, not the
      // trip. See docs/TEST-PLAN-FINANCE.md F9 for why this is no longer
      // APPROVED on submit.
      expect(trip?.status).toBe('PENDING');
    });

    it('cannot decide an already-decided trip', async () => {
      // This used to rely on an earlier case leaving an APPROVED trip behind,
      // which stopped being true when travel stopped approving itself on submit
      // (docs/TEST-PLAN-FINANCE.md F9). Approve one explicitly so the case tests
      // the double-decision guard rather than the state another case happened
      // to leave.
      const trip = await ctx.prisma.travelRequest.findFirst({
        where: { employeeId: empAId, status: { in: ['PENDING', 'APPROVED'] } },
      ...
      if (trip!.status === 'PENDING') {
        await ctx
          .http()
          .post(`/travel-requests/${trip!.id}/approve`)
          .set(bearer(adminToken))
          .send({})
          .expect(201);
      }
