...
// ───────────────────────────────────────────────────────────────────────────
//
// Exported rather than private: every spec that reads a payroll response needs
// the identical semantics, and two copies of "does this response have a `data`
// wrapper" is exactly the drift this module exists to stop.
 ...
 * owned. Every sweeper built on this convention (`clearPayrollLane` here, and
 * the per-spec ones) matches on the PREFIX, so pass them the same literal you
 * passed here, not the marker this returns.
 */
...
const NO_LOGIN =
  ...
  '    (EmployeesService.generateTempPassword) that is only emailed.\n' +
  ...
  '  • Drive the request as ADMIN/HR on the employee\'s behalf (the review routes\n' +
  '    all take the record id, not a session).\n' +
 ...
 * `baseSalary: 0`, and a deduction taken against a zero-pay cycle takes NOTHING.
 * A spec that asserts money was recovered must run against an employee who is
 ...
 * WOULD be permitted to do and which approver-role settings match it — it does
 * not make the account loggable.
 */
export async function makeEmployee(
  ...
  const created = await admin
    .post<{ id: string; employeeCode: string; email: string }>('/employees', {
      ...
      // and keeps every minimum-service rule satisfied whatever it is set to.
 ...
 *
 *   • Clearance blocking is on by default, so an employee who still holds
 *     company assets CANNOT be terminated without an override. The reason is
 *     always supplied here — the alternative is that every offboarding spec
 ...
 * a payroll engine switch mid-suite changes the answer for every spec running in
 * parallel, and the failures land in files that never touched the flag — the
 * worst attribution failure available here. Same convention, and the
 ...
 * unknowable and `withSetting` refuses rather than restoring a guess. Not every
 * key the engine reads is enumerated, so a spec reaching for an exotic one has
 * to check before it flips it.
 */
 ...
 * throws.
 *
 * The `finally` is the whole point. A spec that flips a payroll engine switch
 * and then fails an assertion leaves it on for every worker still
 ...
 * when keys interact — a master switch gates whether the rest are even consulted,
 * so a caller who turned it on first has it turned off last — and independence
 * matters because one key failing to restore must not abandon the remaining four.
 */
 ...
 * is per-branch, and a spec proving that a record in one branch is invisible from
 ...
 *
 * The reason this exists rather than everyone sharing `E2E-BR2`: plenty of the
 * records a spec creates can never be removed. Retirement paths move a row to a
 * terminal state and leave it exactly where it was, so a spec that seeds
 * hundreds of them permanently changes whatever branch it seeded them into —
 * and other specs pick the first non-HO branch and assert it is EMPTY.
 *
...
export interface PayrollRunOpts {
   ...
   * The run type decides which recurring earnings and deductions the engine
   * applies at all, which is exactly what a spec proving "a BONUS run does not
   * charge a monthly deduction twice" needs to vary.
   */
  runType?: string;
  employeeIds?: string[];
}
 ...
 * back from `GET /payrolls/:id`. Every caller wants them — a payroll spec's whole
 * question is what landed on one employee's row — so the second call happens
 * here rather than in thirteen `beforeAll`s.
 *
...
export async function runPayroll(
  ...
  const created = await scoped.post<{ id: string }>('/payrolls', body).catch(async (e: Error) => {
    ...
    // "Attendance for m/yyyy has not been processed yet." A spec picks a
 ...
 * APPROVED → LOCKED.
 *
 * Locking is what settles the run's money: leave encashment, gratuity accrual
 * and garnishment collections all happen here — so a spec asserting any of them
 * has to lock, not merely generate. The run must already be APPROVED (`submit`
 * then `approve`); the deprecated `:id/finalize` alias goes through the same code
 * path and the same requirement.
 */
...
/**
 * LOCKED → APPROVED, reversing what the lock settled on the way.
 *
 ...
 * explainable. Refused with a 409 when a locked revision descends from this
 * payroll — the server protecting an audit chain, not a flake.
 */
 ...
 * revision descends from it, so the unlock is a 409) is
...
const NUMERIC_ITEM_KEYS = [
  ...
  'overtimeHours', 'overtimePay', 'foodAllowance',
  'garnishment', 'insurance', 'tax', 'netSalary',
] as const;
 ...
 * `bankingCountries: []`. Every banking case then fails for a reason that looks
 * like a payroll defect — an employee's bank details are refused because the
 * branch banks in no country at all. Resolve the country from the branch rather
 * than hardcoding one, or the same class of failure arrives from the other
 * direction the first time the seed's country changes.
 *
 ...
 * `subject.netSalary + subject.garnishment === twin.netSalary` — and no case has
 ...
 *
 * Newest period FIRST. `unlockPayroll` is refused with a 409 when a LATER locked
 * revision descends from a run, so sweeping oldest-first wedges on the first
...
export async function ensureCarrier(
  ...
  return makeEmployee(admin, {
    ...
    // the engine and a carrier should be the most boring row in the run.
    baseSalary: 1000,
  });
}
 ...
 * summing across them would let a PF line reconcile against a garnishment.
 */
