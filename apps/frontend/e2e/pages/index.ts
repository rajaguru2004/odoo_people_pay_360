...
export class PayrollManagePage {
  ...
  /**
   * Locking is the step that matters: it is the only one that publishes the
   * payslips, and a LOCKED run cannot be edited. Guarded by the shared confirm
   * dialog.
   */
...
// ─── Workplace: Projects ─────────────────────────────────────────────────────
//
// Two shapes worth knowing before reading these objects:
//
//  * The permission matrix is TRANSPOSED — permissions are rows, roles are
//    columns. `roleHeader(slug)` addresses a column, `permissionRow(key)` a
//    row, and `matrixCell(slug, key)` the checkbox where they meet. That cell
//    is the only handle a spec needs to prove a capability actually moved.
//  * The create form and the create/edit MODAL deliberately share the same
//    `project-form-*` testids, so `ProjectFormPage` drives both screens.

export class ProjectsPage {
  constructor(private readonly page: Page) {}

  async open(): Promise<void> {
    await open(this.page, '/dashboard/projects');
  }
  ...
  get statusFilter() {
    return this.page.getByTestId('project-status-filter');
  }
  get priorityFilter() {
    return this.page.getByTestId('project-priority-filter');
  }
  get newButton() {
    return this.page.getByTestId('project-new');
  }
  get refresh() {
    return this.page.getByTestId('project-refresh');
  }
  get empty() {
    return this.page.getByTestId('project-empty');
  }
  get createFirst() {
    return this.page.getByTestId('project-create-first');
  }

  stat(key: 'total' | 'active' | 'on-hold' | 'completed') {
    return this.page.getByTestId(`project-stat-${key}`);
  }
  card(slug: string) {
  ...
  get anyCard() {
    return this.page.locator('[data-testid^="project-card-"]').first();
  }
  /** Slugs currently rendered — the cheapest way to assert a project is NOT listed. */
  async visibleSlugs(): Promise<string[]> {
    const ids = await this.page
      .locator('[data-testid^="project-card-"]')
      .evaluateAll((els) =>
        els.map((el) => el.getAttribute('data-testid') ?? ''),
      );
    return ids.map((id) => id.replace(/^project-card-/, '')).filter(Boolean);
  }
}

/** Drives `/dashboard/projects/new` AND `NewProjectModal` — identical testids. */
export class ProjectFormPage {
  constructor(private readonly page: Page) {}

  async open(): Promise<void> {
    await open(this.page, '/dashboard/projects/new');
  }
  ...
  get modal() {
    return this.page.getByTestId('project-modal');
  }
  get error() {
    return this.page.getByTestId('project-form-error');
  }

  get name() {
    return this.page.getByTestId('project-form-name');
  }
  get taskPrefix() {
    return this.page.getByTestId('project-form-task-prefix');
  }
  get description() {
    return this.page.getByTestId('project-form-description');
  }
  get startDate() {
    return this.page.getByTestId('project-form-start-date');
  }
  get endDate() {
    return this.page.getByTestId('project-form-end-date');
  }
  get owner() {
    return this.page.getByTestId('project-form-owner');
  }
  get department() {
    return this.page.getByTestId('project-form-department');
  }
  get team() {
    return this.page.getByTestId('project-form-team');
  }
  get submit() {
    return this.page.getByTestId('project-form-submit');
  }
  get cancel() {
    return this.page.getByTestId('project-form-cancel');
  }

  // Segmented pickers: the group, then one option by its VALUE (never its label).
  status(value?: string) {
    return value
      ? this.page.getByTestId(`project-form-status-${value}`)
      : this.page.getByTestId('project-form-status');
  }
  priority(value?: string) {
    return value
      ? this.page.getByTestId(`project-form-priority-${value}`)
      : this.page.getByTestId('project-form-priority');
  }
  visibility(value?: string) {
    return value
      ? this.page.getByTestId(`project-form-visibility-${value}`)
      : this.page.getByTestId('project-form-visibility');
  }

  get memberPicker() {
    return this.page.getByTestId('project-form-member-picker');
  }
  get memberSearch() {
    return this.page.getByTestId('project-form-member-search');
  }
  memberOption(employeeId: string) {
    return this.page.getByTestId(`project-form-member-option-${employeeId}`);
  }
  memberChip(employeeId: string) {
    return this.page.getByTestId(`project-form-member-chip-${employeeId}`);
  }
  get memberDone() {
    return this.page.getByTestId('project-form-member-done');
  }

  /**
   * NOTE: there is no slug field. The slug is server-generated, which is what
   * makes the duplicate-slug auto-increment a server concern rather than a form
   * one — see PRJ-API-33.
   */
  async create(fields: {
    name: string;
    ...
    if (fields.description) await this.description.fill(fields.description);
    if (fields.status) await this.status(fields.status).click();
    if (fields.priority) await this.priority(fields.priority).click();
    if (fields.visibility) await this.visibility(fields.visibility).click();
    await this.submit.click();
    await this.page.waitForLoadState('networkidle').catch(() => {});
  }
}
...
export class ProjectDetailPage {
  ...
  async open(slug?: string): Promise<void> {
    ...
    await open(this.page, `/dashboard/projects/${target}`);
  }
  ...
  tab(
    ...
    return this.page.getByTestId(`project-tab-${name}`);
  }
  async openTab(
    name:
      | 'overview'
  ...
  get notFound() {
    return this.page.getByTestId('project-detail-notfound');
  }
  get editButton() {
    return this.page.getByTestId('project-edit');
  }
  get settingsEdit() {
    return this.page.getByTestId('project-settings-edit');
  }
  /** One button whose testid flips with `isArchived` — assert on which one exists. */
  get archiveButton() {
    return this.page.getByTestId('project-archive');
  }
  get unarchiveButton() {
    return this.page.getByTestId('project-unarchive');
  }

  detail(
    field: 'owner' | 'department' | 'team' | 'visibility' | 'start-date' | 'end-date',
  ) {
    return this.page.getByTestId(`project-detail-${field}`);
  }
  overviewStat(key: 'tasks' | 'members' | 'sprints') {
    return this.page.getByTestId(`project-overview-stat-${key}`);
  }

  /**
   * The delete control, which this object deliberately did not have.
   *
   * R22 was that `DELETE /projects/:id` was enforced behind `PROJECT_DELETE`,
   * covered at the API, and reachable by nobody: the Settings tab offered
   * archive and unarchive only. An accessor was withheld rather than written
   * against an element that did not exist. The control now lives in the
   * Settings danger zone, gated on the same permission, behind a confirmation
   * that names the project and says the delete is soft.
   */
  get deleteButton() {
    return this.page.getByTestId('project-delete');
  }
}

export class ProjectMembersTab {
  constructor(private readonly page: Page) {}

  get table() {
    return this.page.getByTestId('member-table');
  }
  get empty() {
    return this.page.getByTestId('member-empty');
  }
  row(memberId: string) {
    return this.page.getByTestId(`member-row-${memberId}`);
  }
  roleSelect(memberId: string) {
    return this.page.getByTestId(`member-role-select-${memberId}`);
  }
  removeButton(memberId: string) {
    return this.page.getByTestId(`member-remove-${memberId}`);
  }

  get addEmployee() {
    return this.page.getByTestId('member-add-employee');
  }
  get addRole() {
    return this.page.getByTestId('member-add-role');
  }
  get addButton() {
    return this.page.getByTestId('member-add');
  }

  get anyRow() {
    return this.page.locator('[data-testid^="member-row-"]').first();
  }
}

/**
 * The permission matrix. Transposed: permissions are ROWS, roles are COLUMNS.
 *
 * `matrixCell` is the load-bearing accessor — a journey toggles one exact cell
 * and then proves the member's capability actually changed by calling the API
 * as that member. Anything less is testing a checkbox, not a permission.
 */
export class ProjectRolesManagerPage {
  constructor(private readonly page: Page) {}

  get matrix() {
    return this.page.getByTestId('role-matrix');
  }
  get error() {
    return this.page.getByTestId('role-error');
  }
  get saveButton() {
    return this.page.getByTestId('role-save');
  }
  get createButton() {
    return this.page.getByTestId('role-create');
  }
  get dirtyCount() {
    return this.page.getByTestId('role-dirty-count');
  }

  roleHeader(roleSlug: string) {
    return this.page.getByTestId(`role-header-${roleSlug}`);
  }
  permissionRow(permission: string) {
    return this.page.getByTestId(`role-permission-row-${permission}`);
  }
  matrixCell(roleSlug: string, permission: string) {
    return this.page.getByTestId(`role-matrix-cell-${roleSlug}-${permission}`);
  }
  deleteRole(roleSlug: string) {
    return this.page.getByTestId(`role-delete-${roleSlug}`);
  }

  get createModal() {
    return this.page.getByTestId('role-create-modal');
  }
  get createName() {
    return this.page.getByTestId('role-create-name');
  }
  get createCopyFrom() {
    return this.page.getByTestId('role-create-copy-from');
  }
  get createSubmit() {
    return this.page.getByTestId('role-create-submit');
  }
  get createCancel() {
    return this.page.getByTestId('role-create-cancel');
  }
  get createError() {
    return this.page.getByTestId('role-create-error');
  }

  /** Toggles one capability and saves. The spec then re-checks it over the API. */
  async toggle(roleSlug: string, permission: string): Promise<void> {
    await this.matrixCell(roleSlug, permission).click();
...
export class KanbanPage {
  ...
  column(statusId: string) {
    return this.page.getByTestId(`kanban-column-${statusId}`);
  }
  /** The dnd-kit droppable — the drop TARGET, not the column shell. */
  dropzone(statusId: string) {
    return this.page.getByTestId(`kanban-column-dropzone-${statusId}`);
  }
  columnCount(statusId: string) {
    return this.page.getByTestId(`kanban-column-count-${statusId}`);
  }
  columnEmpty(statusId: string) {
    return this.page.getByTestId(`kanban-column-empty-${statusId}`);
  }
  addInColumn(statusId: string) {
    return this.page.getByTestId(`kanban-column-add-${statusId}`);
  }

  /** Carries the dnd-kit listeners — this element IS the drag handle. */
  card(taskCode: string) {
    return this.page.getByTestId(`task-card-${taskCode}`);
  }

  /**
   * Drags a card into a column.
   *
   * dnd-kit does not respond to Playwright's `dragTo`: it listens for pointer
   * events, and a synthesised HTML5 drag never reaches it. The sequence below
   * (press, a first small move to cross the activation threshold, a move over
   * the target, release) is what the library's own tests use.
   */
  async dragCardTo(taskCode: string, statusId: string): Promise<void> {
    const card = this.card(taskCode);
    const target = this.dropzone(statusId);
    await card.scrollIntoViewIfNeeded();
    const from = await card.boundingBox();
    const to = await target.boundingBox();
    if (!from || !to) throw new Error(`drag: missing box for ${taskCode} → ${statusId}`);

    await this.page.mouse.move(from.x + from.width / 2, from.y + from.height / 2);
    await this.page.mouse.down();
    await this.page.mouse.move(from.x + from.width / 2 + 12, from.y + from.height / 2 + 12, {
      steps: 4,
    });
    await this.page.mouse.move(to.x + to.width / 2, to.y + Math.min(60, to.height / 2), {
      steps: 12,
    });
...
export class TaskDetailPage {
  ...
  get drawer() {
    return this.page.getByTestId('task-drawer');
  }
  get drawerClose() {
    return this.page.getByTestId('task-drawer-close');
  }
  get notFound() {
    return this.page.getByTestId('task-detail-notfound');
  }
  get code() {
    return this.page.getByTestId('task-detail-code');
  }
  get title() {
    return this.page.getByTestId('task-detail-title');
  }
  get titleInput() {
    return this.page.getByTestId('task-detail-title-input');
  }
  get saveButton() {
    return this.page.getByTestId('task-save');
  }
  get deleteButton() {
    return this.page.getByTestId('task-delete');
  }
  ...
  field(
    ...
    return this.page.getByTestId(`task-detail-${name}`);
  }
  edit(
    name: 'type' | 'sprint' | 'priority' | 'status' | 'dates' | 'assignees' | 'reporter' | 'labels',
  ) {
    return this.page.getByTestId(`task-detail-${name}-edit`);
  }
  statusOption(statusId: string) {
    return this.page.getByTestId(`task-status-option-${statusId}`);
  }
  priorityOption(priority: string) {
    return this.page.getByTestId(`task-priority-option-${priority}`);
  }
  assigneeOption(employeeId: string) {
    return this.page.getByTestId(`task-assignee-option-${employeeId}`);
  }

  /** Opens the status picker, picks a column, and lets the write settle. */
  async setStatus(statusId: string): Promise<void> {
    await this.edit('status').click();
    await this.statusOption(statusId).click();
    await this.page.waitForLoadState('networkidle').catch(() => {});
  }

  // Sub-panels.
  comment(id: string) {
    return this.page.getByTestId(`comment-row-${id}`);
  }
  get commentInput() {
    return this.page.getByTestId('comment-input');
  }
  get commentSubmit() {
    return this.page.getByTestId('comment-submit');
  }
  get commentEmpty() {
    return this.page.getByTestId('comment-empty');
  }
  attachment(id: string) {
    return this.page.getByTestId(`attachment-row-${id}`);
  }
  get attachmentInput() {
    return this.page.getByTestId('attachment-input');
  }
  get attachmentEmpty() {
    return this.page.getByTestId('attachment-empty');
  }
  subtask(taskCode: string) {
    return this.page.getByTestId(`subtask-row-${taskCode}`);
  }
  dependency(taskCode: string) {
    return this.page.getByTestId(`dependency-row-${taskCode}`);
  }
  get dependencyEmpty() {
    return this.page.getByTestId('dependency-empty');
  }
}

export class SprintsTab {
  constructor(private readonly page: Page) {}

  get empty() {
    return this.page.getByTestId('sprint-empty');
  }
  get createButton() {
    return this.page.getByTestId('sprint-create');
  }
  row(id: string) {
    return this.page.getByTestId(`sprint-row-${id}`);
  }
  status(id: string) {
    return this.page.getByTestId(`sprint-status-${id}`);
  }
  startButton(id: string) {
    return this.page.getByTestId(`sprint-start-${id}`);
  }
  completeButton(id: string) {
    return this.page.getByTestId(`sprint-complete-${id}`);
  }
  deleteButton(id: string) {
    return this.page.getByTestId(`sprint-delete-${id}`);
  }

  get form() {
    return this.page.getByTestId('sprint-form');
  }
  get formName() {
    return this.page.getByTestId('sprint-form-name');
  }
  get formGoal() {
    return this.page.getByTestId('sprint-form-goal');
  }
  get formStart() {
    return this.page.getByTestId('sprint-form-start-date');
  }
  get formEnd() {
    return this.page.getByTestId('sprint-form-end-date');
  }
  get formSubmit() {
    return this.page.getByTestId('sprint-form-submit');
  }
  get formCancel() {
    return this.page.getByTestId('sprint-form-cancel');
  }
}
...
export class WorkflowSettingsPage {
  ...
  get openButton() {
    return this.page.getByTestId('workflow-settings-open');
  }
  get modal() {
    return this.page.getByTestId('workflow-settings-modal');
  }
  get done() {
    return this.page.getByTestId('workflow-settings-done');
  }

  row(statusId: string) {
    return this.page.getByTestId(`status-row-${statusId}`);
  }
  name(statusId: string) {
    return this.page.getByTestId(`status-name-${statusId}`);
  }
  nameInput(statusId: string) {
    return this.page.getByTestId(`status-name-input-${statusId}`);
  }
  category(statusId: string) {
    return this.page.getByTestId(`status-category-${statusId}`);
  }
  editButton(statusId: string) {
    return this.page.getByTestId(`status-edit-${statusId}`);
  }
  saveButton(statusId: string) {
    return this.page.getByTestId(`status-save-${statusId}`);
  }
  deleteButton(statusId: string) {
    return this.page.getByTestId(`status-delete-${statusId}`);
  }
  /** Reorder is drag-only — there are no up/down buttons on this modal. */
  dragHandle(statusId: string) {
    return this.page.getByTestId(`status-drag-handle-${statusId}`);
  }

  get createButton() {
    return this.page.getByTestId('status-create');
  }
  get addName() {
    return this.page.getByTestId('status-name');
  }
  get addCategory() {
    return this.page.getByTestId('status-category');
  }
  get addSubmit() {
    return this.page.getByTestId('status-add-submit');
  }
  get addCancel() {
    return this.page.getByTestId('status-add-cancel');
  }
}

export class ProjectTasksViewPage {
  constructor(private readonly page: Page) {}

  get listView() {
    return this.page.getByTestId('project-view-list');
  }
  get kanbanView() {
    return this.page.getByTestId('project-view-kanban');
  }
  get ganttView() {
    return this.page.getByTestId('project-view-gantt');
  }
  get newTask() {
    return this.page.getByTestId('task-new');
  }
  get list() {
    return this.page.getByTestId('task-list');
  }
  get listEmpty() {
    return this.page.getByTestId('task-list-empty');
  }
  taskRow(taskCode: string) {
    return this.page.getByTestId(`task-row-${taskCode}`);
  }
}

export class ProjectActivityLogPage {
  constructor(private readonly page: Page) {}
  ...
  get empty() {
    return this.page.getByTestId('activity-empty');
  }
  row(id: string) {
    return this.page.getByTestId(`activity-row-${id}`);
  }
  get next() {
    return this.page.getByTestId('activity-next');
  }
  get prev() {
    return this.page.getByTestId('activity-prev');
  }
}
