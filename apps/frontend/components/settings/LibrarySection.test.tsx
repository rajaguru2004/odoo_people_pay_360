...
vi.mock('@/services/libraryService', () => ({ default: api }));
...
beforeEach(() => {
  ...
  api.list.mockImplementation((query: { type?: LibraryTypeValue }) =>
    Promise.resolve({ success: true, data: ROWS[query.type ?? 'POSITION'] ?? [] }),
  );
...
describe('Library editor', () => {
  it('opens on positions and lists what is in that library', async () => {
    ...
    expect(api.list).toHaveBeenCalledWith({ type: 'POSITION' });
  });
