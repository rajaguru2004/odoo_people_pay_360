...
/**
 * R20 — the banner has to NAME the blocker, not only report one.
 *
 * `cleared: false` with an empty `openAssets` used to render "Blocked: 0
 * company assets not returned", followed by advice to record a return for an
 * employee with no asset out: the one screen whose entire job is to name the
 * blocker naming the wrong one, and offering a remedy that could not work.
 */
describe('a block it cannot itemise', () => {
  it('never claims a count it does not have', async () => {
    settle({ cleared: false, assetCleared: false, openAssets: [] });
    ...
    expect(status).toHaveTextContent('clearance obligations are outstanding');
  });

  it('survives a payload with no asset list at all', async () => {
    // A fixture written by hand. The banner under-reports rather than throwing,
    // which is the only acceptable failure mode for a screen an approver reads
    // before signing off an exit.
    settle({ cleared: false } as ClearanceStatus);
