...
import { CheckCircle2, Copy, Hash, Loader2, Link2, Unlink } from 'lucide-react';
import { toast } from 'sonner';
import discordService from '@/services/discordService';
import { DiscordLinkCode, MyDiscordStatus } from '@/types/discord';

/**
 * Employee self-service Discord link.
 *
 * The Discord ID is never typed in here on purpose. A snowflake is public — any
 * employee could paste a colleague's and receive their HR notifications. So the
 * browser issues a one-time code and Discord redeems it: the ID is *learned*
 * from a signed interaction rather than claimed in a form, and the field below
 * is read-only proof of what was learned.
 */
export default function DiscordLinkSection() {
  const [status, setStatus] = useState<MyDiscordStatus | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      const res = await discordService.me();
  ...
  // employee runs /link — without asking them to come back and refresh.
  ...
  async function issueCode() {
    setBusy(true);
    try {
      const res = await discordService.startLink();
      setCode(res.data ?? null);
      setSecondsLeft((res.data?.expiresInMinutes ?? 15) * 60);
    } catch (e: any) {
      toast.error(e?.response?.data?.message ?? 'Could not create a link code');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div id="discord" className="rounded-xl border border-slate-200 bg-white p-5">
      <div className="mb-4 flex items-start gap-3">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600">
          <Hash className="h-5 w-5" />
        </div>
        <div>
          <h3 className="font-semibold text-slate-800">Discord</h3>
          <p className="text-sm text-slate-500">
            Link your Discord account to check in, request leave and receive HR updates as slash
            commands.
          </p>
        </div>
      </div>
      ...
      {status.linked ? (
        <div className="space-y-3">
          <div className="flex items-center gap-2 rounded-lg bg-emerald-50 p-3 text-sm text-emerald-800">
            ...
            <span>
              Linked to <span className="font-medium">{status.discordTag ?? 'your account'}</span>
            </span>
          </div>

          <div>
            <label className="text-xs font-medium text-slate-500">Discord ID</label>
            <input
              readOnly
              value={status.discordUserId ?? ''}
            ...
            <p className="mt-1 text-xs text-slate-400">
              Set by Discord when you ran <code>/link</code>. It cannot be typed in — that is what
              stops someone else&apos;s ID being entered here.
            </p>
          </div>
        ...
        <div className="space-y-3">
          <div className="rounded-lg bg-slate-50 p-4">
            <p className="text-sm text-slate-500">In Discord, send</p>
        ...
        <div className="space-y-3">
          ...
          <p className="text-xs text-slate-400">
            You will get a six-digit code to run as <code>/link &lt;code&gt;</code> in Discord.
