// ISO 3166-1 alpha-2 country list for pickers (code + English name + dial code).
export interface Country {
  code: string;
  name: string;
  /** International calling code without the '+', e.g. '968'. Generated from
   *  libphonenumber-js metadata; not unique (NANP countries all share '1'). */
  dial: string;
}
...
/** Calling code without '+' for an ISO-2 code, or '' when unknown. */
export function countryDial(code?: string | null): string {
  if (!code) return '';
  return COUNTRIES.find((x) => x.code === code.toUpperCase())?.dial ?? '';
}

/**
 * Countries a multi-country workforce is most often hired from, floated to the
 * top of phone pickers. The full list still follows — this is ordering only,
 * never a restriction.
 */
export const PHONE_COUNTRY_PRIORITY = [
  'IN', 'AE', 'SA', 'OM', 'QA', 'KW', 'BH', 'SG', 'MY', 'PH',
  'LK', 'BD', 'NP', 'PK', 'GB', 'US',
];

/** COUNTRIES with the priority set first, each still carrying its dial code. */
export const PHONE_COUNTRIES: Country[] = [
  ...PHONE_COUNTRY_PRIORITY.map((c) => COUNTRIES.find((x) => x.code === c)!).filter(Boolean),
  ...COUNTRIES.filter((c) => !PHONE_COUNTRY_PRIORITY.includes(c.code)),
];
